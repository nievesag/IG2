...
bool IG2App::keyPressed(const OgreBites::KeyboardEvent& evt){
    ...
    if (evt.keysym.sym == SDLK_c) {
        emittingSphere = !emittingSphere;
        pSysSphere->setEmitting(emittingSphere);
    }
...
void IG2App::setupScene(void){
    ...
    */
    ...
    pSys = mSM->createParticleSystem("psSmoke", "Examples/Smoke");
    mPSNode = mSM->getRootSceneNode()->createChildSceneNode();
    mPSNode->attachObject(pSys);
    mPSNode->setPosition(0,30,0);
    pSys->setEmitting(emitting);

    // sphere
    sphereEnt = mSM->createEntity("sphere.mesh");
    sphereNode = mSM->getRootSceneNode()->createChildSceneNode();
    sphereNode->attachObject(sphereEnt);
    sphereNode->scale(0.1, 0.1, 0.1);
    sphereNode->setPosition(Vector3(100, 50, 0)); // On the floor!
    sphereNode->setInitialState();

    pSysSphere = mSM->createParticleSystem("psSmokeSphere", "Examples/Smoke");
    mPSNodeSphere = mSM->getRootSceneNode()->createChildSceneNode();
    sphereNode->attachObject(pSysSphere);
    pSysSphere->setEmitting(emittingSphere);
...
{   
    // rota esfera
    //sphereNode->rotate(, Ogre::Node::TS_WORLD);
}
